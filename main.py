import tkinter as tk  # 创建GUI应用
from tkinter import messagebox  # 创建消息框
import random  # 生成随机数

# 定义全局变量
FPS = 500  # 页面刷新间隔（ms） 间隔越小速度越快
R = 20  # 行数
C = 12  # 列数
cell_size = 30  # 方格大小
height = R*cell_size  # 画布的高度
width = C*cell_size  # 画布的宽度
...
def draw_cell_by_cr(canvas, c, r, color="#CCCCCC"):
    ...
    :param c: 方格所在列
    :param r: 方格所在行
    :param color: 方格的颜色，默认值为#CCCCCC，青灰色
    :return:
    """
    x0 = c*cell_size
    y0 = r*cell_size
    ...
    # 在画板上创建一个矩形表示方格，width线条粗细，outline线条颜色，fill方格颜色
...
def draw_cells(canvas, c, r, cell_list, color="#CCCCCC"):
    ...
    :param c: 该形状设定的原点所在列
    :param r: 该形状设定的原点所在行
    :param cell_list: 该形状各个方格相对于自身原点所处的位置坐标
    :param color: 该形状颜色
    :return:
    """
...
def generate_new_block():  # 随机生成新的俄罗斯方块
    ...
    kind = random.choice(list(SHAPES.keys()))  # 使用random库里的choice语句来随机一个方块列表里的元素
    cr = [C//2, 0]  # 初始行列在第一行的正中间
    new_block = {
        "kind": kind,
